...
macro_rules! binary_scalar_operation(
    ($operation:literal, $operator:literal, $mth:ident, $doc:expr) => (
        fn $mth(self, x: B) -> Self::Output {
            ...
            let scalar_buffer = self.data.wgpu_device.create_storage_buffer(&[x]);
...
macro_rules! binary_scalar_infix(
    ($operator:literal, $mth:ident, $doc:expr) => (
        binary_scalar_operation!("result.data[id] = lhs.data[$lhs_offset + lhs_id] $op scalar;", $operator, $mth, $doc);
    )
);
